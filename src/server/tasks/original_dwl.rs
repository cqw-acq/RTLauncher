...
async fn download_task(
    ...
    loop {
        // Check existing file integrity first
        if let Ok(mut file) = File::open(&task.target_path).await {
            match check_sha1(&mut file, &task.sha1).await {
                ...
                Ok(false) => {
                    eprintln!("File verification failed, triggering re-download: {}", task.target_path.display());
        ...
        match result {
            Ok(_) => {
                ...
                if check_sha1(&mut file, &task.sha1).await? {
                    ...
                    eprintln!("Post-download verification failed, triggering retry");
                    fs::remove_file(&task.target_path)?;
                }
            }
            Err(e) => {
                eprintln!("Download failed [{}]: {}", url_used, e);
                if let Some(p) = &progress {
...
async fn download_with_url(
    ...
    if task.size > 0 && content_length != task.size {
        return Err(format!("File size mismatch: expected {} actual {}", task.size, content_length).into());
    }

    // Use complete byte buffer instead of stream write
    let bytes = response.bytes().await?;
    
    // Atomic write: write to temp file first, rename after verification
    ...
    if !check_sha1(&mut file, &task.sha1).await? {
        return Err("Post-download verification failed".into());
    }
...
pub async fn process_version(
    ...
    // Save index file (use asset_index.id as filename)
    ...
    for lib in version_data.libraries {
        ...
        // Process main library file
        if let Some(artifact) = lib.downloads.artifact {
            let original_url = format!("{}/{}", base_url, artifact.path);
        ...
        // Process platform-specific libraries
        if let Some(classifier_template) = lib.natives.get(os_type) {
            ...
            if let Some(native) = lib.downloads.classifiers.get(&classifier) {
                println!("Downloading platform-specific library: {}", native.path);
...
async fn fetch_version_url(version: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    let client = reqwest::Client::new();
