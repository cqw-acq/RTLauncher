//! Server module for RTLauncher backend functionality
//! 
//! This module provides the backend server with:
//! - Task queue for download operations
//! - WebSocket support for real-time communication
//! - Minecraft version management and downloads
...
fn spawn_worker(queue: Data<Queue>, sessions: Data<Sessions>) {
    task::spawn(async move {
        loop {
            // 1. Get first task from queue
            let job = {
                let mut q = queue.lock().await;
                q.queue.pop_front()
            };

            // Queue empty - mark as stopped & exit
            let job = match job {
                Some(j) => j,
                None => {
            ...
            // 4. Push to corresponding WebSocket session
